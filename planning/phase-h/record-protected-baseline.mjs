...
mkdirSync(resolve(root, "planning/phase-h"), { recursive: true });
writeFileSync(resolve(root, "planning/phase-h/PROTECTED-WORK-BASELINE.json"), JSON.stringify({ schema_version: "phase-h.protected-work-baseline.v1", captured_at: new Date().toISOString(), repository: "Ix-findings", branch: run("git", ["branch", "--show-current"]), head: run("git", ["rev-parse", "HEAD"]), dirty: changedFiles.length > 0, changed_file_count: changedFiles.length, changed_files: changedFiles, external_mutations_allowed: 0 }, null, 2) + "\n");
