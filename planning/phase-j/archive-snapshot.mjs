...
const dir = resolve(root, "planning/phase-j/source-snapshots");
mkdirSync(dir, { recursive: true });
const archive = resolve(dir, `${version}.json`);
if (!existsSync(archive)) copyFileSync(mirrorPath, archive);
const manifestPath = resolve(root, "planning/phase-j/SNAPSHOT-MANIFEST.json");
const manifest = existsSync(manifestPath) ? JSON.parse(readFileSync(manifestPath, "utf8")) : { schema_version: "phase-j.snapshot-manifest.v1", snapshots: [] };
if (!manifest.snapshots.some(item => item.snapshot_version === (mirror.snapshot_version || mirror.generated_at))) manifest.snapshots.push({ snapshot_version: mirror.snapshot_version || mirror.generated_at, file: `planning/phase-j/source-snapshots/${version}.json`, source: mirror.source_capture, archived_at: new Date().toISOString() });
