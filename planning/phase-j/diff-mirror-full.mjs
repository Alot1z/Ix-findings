...
const FORCE_PREVIOUS = process.env.DIFF_FROM_SNAPSHOT || null;
const previousEntry = FORCE_PREVIOUS ? (manifest.snapshots || []).find(item => item.snapshot_version === FORCE_PREVIOUS) : [...(manifest.snapshots || [])].reverse().find(item => item.snapshot_version === current.previous_snapshot_version);
...
writeFileSync(resolve(root, "planning/phase-j/MIRROR-DIFF-FULL.json"), JSON.stringify(report, null, 2) + "\n");
const lines = [`# Phase J Mirror Diff (full baseline 2026-08-12T13:07:16 -> current)`, ``, `Generated: ${report.generated_at}`, ``, `Source authority: **GITHUB**`, ``, `Before: ${report.snapshot_before || "UNKNOWN"}`, `After: ${report.snapshot_after || "UNKNOWN"}`, ``, `Changed objects: **${report.changed_count}**`, `Unchanged objects: **${report.unchanged_count}**`, ``, `## Change classes`, ``];
...
writeFileSync(resolve(root, "planning/phase-j/MIRROR-DIFF-FULL.md"), lines.join("\n") + "\n");
