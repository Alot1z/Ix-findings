...
const manifest = read("planning/phase-e/SNAPSHOT-MANIFEST.json");
...
const report = {
  schema_version: "phase-e.mirror-diff.v1",
...
writeFileSync(resolve(root, "planning/phase-e/MIRROR-DIFF.json"), JSON.stringify(report, null, 2) + "\n");
const lines = [`# Phase E Mirror Diff`, ``, `Generated: ${report.generated_at}`, ``, `Source authority: **GITHUB**`, ``, `Before: ${report.snapshot_before || "UNKNOWN"}`, `After: ${report.snapshot_after || "UNKNOWN"}`, ``, `Changed objects: **${report.changed_count}**`, `Unchanged objects: **${report.unchanged_count}**`, ``, `## Change classes`, ``];
...
writeFileSync(resolve(root, "planning/phase-e/MIRROR-DIFF.md"), lines.join("\n") + "\n");
