...
The gate also enforces **superseded-commit staleness**: any manifest-era commit entity (`CLI-HANDOFF/manifest.json` `commits`) still marked `CURRENT` in the canonical graph while the live capture (`knowledge/live-github-state.json`) records a newer head for the same branch fails the gate. Live heads come from open-PR heads first, then from the captured `fork_branches` block (repo-guarded), so branches without an open PR (e.g. `feat/ix-agent-skill`) are also covered. The canonical builder reconciles such commits to `HISTORICAL`; if any remain `CURRENT`, the graph is presenting a superseded commit as live state and publication must stop. Example observed check: `superseded-commit:1497596` must be `HISTORICAL` once the live head of `feat/ix-remap-hardening` is `a6a4726` (and `superseded-commit:b038c46` once `feat/ix-agent-skill` is `30f28b3`).

The gate also enforces **purged-SHA hygiene**: commits physically purged from the local clones (listed in `knowledge/purged-commits.json`) must never appear in a live/current position. `knowledge/purged-sha-audit.mjs` (`auditPurgedShas`, a pure function) fails when a purged SHA shows up in a live-status entity's metadata (outside labeled provenance keys) or summary, in a non-`HISTORICAL` relationship endpoint, in live-capture fields (implementation file refs, implementation head/commit refs, open-PR heads, fork-branch heads), or as a `COMMIT-<sha>` entity that is not `HISTORICAL`. Labeled provenance (`historical_sha`, `superseded_at`, `historical_note`, …) and `HISTORICAL` records are deliberately exempt — they document what existed, not what is current. Observed checks look like `purged-sha:entity:BRANCH-…:historical_sha` (exempt) and would fail as `purged-sha:commit:1497596` if a purged commit were ever presented as `CURRENT` again.
