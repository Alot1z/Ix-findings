...
for (const r of manifest.branches || []) { const current = r.repo === "ix-infrastructure/Ix" && r.branch === "main" ? { ...r, sha: liveHead, historical_sha: r.sha } : r; entity("BRANCH", `BRANCH-${idHash(`${r.repo}-${r.branch}`)}`, `${r.repo}:${r.branch}`, { status: current.dirty ? "IN_PROGRESS" : "CURRENT", confidence: "HIGH", human_summary: `${r.repo}:${r.branch} at ${current.sha || UNKNOWN}.`, source_refs: sourceRefs("CLI-HANDOFF/manifest.json", "CLI-HANDOFF/phase-15/FINAL-PR-WORTHINESS-AUDIT.json"), metadata: current }); }
...
entity("IMPLEMENTATION", "IMPL-REMAP-PR393", "Remap hardening implementation", { status: "PR_OPEN", confidence: "HIGH", source_refs: sourceRefs("CLI-HANDOFF/phase-15/IMPLEMENTATION-QUALITY-MATRIX.json", "pr-packets/ix-remap-hardening/README.md"), human_summary: "Loopback-guarded POST /__ix/remap implementation carried by open PR #393.", deep_summary: "The implementation includes loopback/Host/Origin controls, WSL bootstrap changes, guard tests, and a map subprocess. Phase 15 retains lifecycle and concurrency review concerns.", metadata: { pr: 393, branch: "feat/ix-remap-hardening", head: "1497596", known_limitations: ["descendant cleanup requires stronger proof", "concurrent expensive requests are not explicitly bounded"] } });
